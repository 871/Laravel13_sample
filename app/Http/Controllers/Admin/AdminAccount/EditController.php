...
class EditController extends Controller
{
    public function index(Request $request, $admin_account_id)
    {
        ...
        return redirect()->to(sprintf('%s/edit/%s/input', $request->path(), $inputProcess->getId()));
    }
    ...
    public function inputPost(Request $request, $process_id)
    {
        ...
        try {
            ...
            return redirect()->to(sprintf('%s/edit/%s/conf', $request->path(), $process_id));
    ...
    public function confPost(Request $request, $process_id)
    {
        ...
        try {
            ...
            return redirect()->route('admin.admin_account.index', ['account_id' => $request->route('account_id')])->with('success', '管理者アカウントの更新が完了しました。');
        } catch (ValidateException $ex) {
            $service->inputProcessErrorUpdate($ex);
